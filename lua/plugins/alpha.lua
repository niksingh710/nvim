return {
	...
	config = function()
		...
		if utils.check.file_exists(os.getenv("HOME") .. "/.config/nvim/lua/config.lua") then
			...
			table.insert(
				...
				dashboard.button(
					"c",
					"⚯  Quick Config",
					":e ~/.config/nvim/lua/config.lua <CR><esc>:lcd %:p:h<cr>"
				)
			)
		end
		...
		if utils.check.file_exists(os.getenv("HOME") .. "/.config/waybar/bars/top.json") then
			table.insert(
				...
				dashboard.button(
					"W",
					icons.ui.Ellipsis .. "  waybar Config",
		...
		local vok, val = pcall(require, "alpha-headers")
		if vok then
